...
import type { ShapeProvider, ShapeParams } from "@/hooks/particle-shapes"
import type { UseParticlesResult, FrameHook } from "@/hooks/use-particles"
...
export interface UseShapes3DResult {
  ...
  /** Nudge rotation by an impulse — good for OSC/MIDI hits. */
...
export function useShapes3D(
  ...
  // Pre-cached vertex arrays per shape name + count key so we don't
  // regenerate geometry every frame when count is stable.
  const vertCacheRef = useRef<Map<string, Vec3[]>>(new Map())

  // Auto-rotation frame hook
  ...
  // -------------------------------------------------------------------------
  // getProvider — returns a ShapeProvider that projects the 3D shape
  // into the current viewport using the live rotation state.
  // -------------------------------------------------------------------------
