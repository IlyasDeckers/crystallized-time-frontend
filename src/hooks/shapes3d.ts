import type { Vec3 } from "@/hooks/math3d"
...
export function sphereVerts(n = 80): Vec3[] {
  const verts: Vec3[] = []
  // Distribute evenly using Fibonacci sphere for nicer spread
...
export function torusVerts(n = 100, R = 1, r = 0.35): Vec3[] {
  const verts: Vec3[] = []
  // Distribute points evenly across the torus surface
...
export function icosphereVerts(n = 80, subdivisions = 2): Vec3[] {
  ...
  for (let s = 0; s < subdivisions; s++) {
    ...
    faces = newFaces
    // Deduplicate verts that landed on the same sphere point
...
export function helixVerts(n = 80, turns = 4): Vec3[] {
  ...
  // Cross-links every ~quarter turn
...
// ---------------------------------------------------------------------------
// Klein bottle (approximation — immersed in 3D with self-intersection)
// ---------------------------------------------------------------------------
